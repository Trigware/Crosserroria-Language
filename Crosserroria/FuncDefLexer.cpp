...
bool Lexer::DetermineIfAssigning(std::string symbolName) {
	...
	if (determiningIfIsAssignment) {
		...
		if (functionLevelMember.underlyingExpression.has_value()) {
			Expression& expr = functionLevelMember.underlyingExpression.value();
			isValidBinOp = std::find(expr.validBinaryOperators.begin(), expr.validBinaryOperators.end(), expr.currentBinaryOperator) != expr.validBinaryOperators.end();
		}
...
void Lexer::InitializeFunctionLevelMember() {
	functionLevelMember = FunctionLevelInstruction(currentNestingLevel);
	functionLevelSymbolHistory.clear();
}

FunctionLevelInstruction::FunctionLevelInstruction(int nestingLevel) {
	underlyingExpression = Expression();
	instructionType = InstructionType::PlainExpression;
